function getdatadwellingfactorhhrural(results) {
    ...
    if (maintypes['id']=='Hou'){
                ...
                datar.push(data); 
                    for(k = 0; k < subtypechunk.length; k++){
                        if (subtypechunk!=undefined && subtypechunk!=''){
                            ...
                            for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata[subtypechunk[k]+'_'+dweltypes['id']+'_'+allyear[y]]);} 
                            data['chart']=false;
                            datar.push(data); 
                        }
                }
        ...
        jQuery.each(houendtype,function(i,houendtypes)
        {
            if (houendtypes['id']=='SH' && bjdata['SH_'+typechunk[j]]=='Y'){ 
                ...
                data['unit']='%';
                for(var y=0; y<allyear.length; y++){data[' '+allyear[y]]=isNumber(bldata['Shr_'+typechunk[j]+'_'+allyear[y]]);}
                data['chart']=false;
                datar.push(data); 
                ...
                for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata['Deg_'+typechunk[j]+'_'+allyear[y]]);}
                data['chart']=false;
                datar.push(data); 
    ...
    //Penetration
     ...
     data['css']='readonly bold';
     data['readonly']=true;
        for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata['P_'+typechunk[j]+'_'+houendtypes['id']+'_'+allyear[y]]);}
     data['chart']=false;
     datar.push(data); 
    ...
    jQuery.each(houtype,function(i,houtypes)
    {
        if(houtypes[houendtypes['id']]=='Y' ){
            if(houtypes['id']=='EL' && houendtypes['id'] !='CO' && houendtypes['id'] !='AC' )
            {
                ...
                data['unit']='%';
                for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata['P_'+typechunk[j]+'_'+houtypes['id']+'_'+houendtypes['id']+'_'+allyear[y]]);}
                data['chart']=false;
                datar.push(data); 
                ...
                data['unit']='%';
                for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata['PH_'+typechunk[j]+'_'+houtypes['id']+'_'+houendtypes['id']+'_'+allyear[y]]);}
                data['chart']=false;
                datar.push(data); 
            }
            else
    ...
    jQuery.each(houtype,function(i,houtypes)
    {
        if(houtypes[houendtypes['id']]=='Y' && houtypes['id']!='DH' ){
            if(houendtypes['id']=='CO' && houtypes['id'] =='EL'){
	        }else{
                var unit='%';
                ...
                data['unit']=unit;
                for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata['E_'+typechunk[j]+'_'+houtypes['id']+'_'+houendtypes['id']+'_'+allyear[y]]);}
                data['chart']=false;
                datar.push(data);   
            }
        }
       ...
       jQuery.each(dweltype,function(i,dweltypes)
        {
           if (dweltypes['ftype']=='WH' && dweltypes['id']=='c'){ 
                ...
                for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata[typechunk[j]+'_'+dweltypes['id']+'_'+allyear[y]]);}
                data['chart']=false;
                datar.push(data);  
           }
               ...
               // for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]='';}
                ...
                for(k = 0; k < subtypechunk.length; k++){
                    if (subtypechunk!=undefined && subtypechunk!=''){
                        var item=doc_clients_data.getElementsByTagName(subtypechunk[k])[0].childNodes[0].nodeValue;
                        var unit='';
                        if(dweltypes['id']=='a'){unit=defaultene;}
                        var data = new Array();
                        data['id']=subtypechunk[k]+'_'+dweltypes['id'];
                        data['item']=item;
                        data['unit']=unit+dweltypes['unit'];  
              ...
              //  for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]='';}
                ...
                for(k = 0; k < subtypechunk.length; k++){
                    if (subtypechunk!=undefined && subtypechunk!=''){
                        var item=doc_clients_data.getElementsByTagName(subtypechunk[k])[0].childNodes[0].nodeValue;
                        var unit='';
                        if(dweltypes['id']=='l'){unit=defaultene;}
                        var data = new Array();
                        data['id']=subtypechunk[k]+'_'+dweltypes['id'];
                        data['item']=item;
                        data['unit']=unit+dweltypes['unit'];  
           ...
           if (dweltypes['ftype']=='AP' && dweltypes['id']!='e'){     
                ...
                for(k = 0; k < subtypechunk.length; k++){
                    if (subtypechunk!=undefined && subtypechunk!=''){
                        var item=doc_clients_data.getElementsByTagName(subtypechunk[k])[0].childNodes[0].nodeValue;
                        var unit='';
                        if(dweltypes['id']=='d' || dweltypes['id']=='f'){unit=defaultene;}
                        var data = new Array();
                        data['id']=subtypechunk[k]+'_LH_'+dweltypes['id'];
                        data['item']=item;
                        data['unit']=unit+dweltypes['unit'];  
                        for(var y=0; y<allyear.length; y++){ data[' '+allyear[y]]=isNumber(bldata[subtypechunk[k]+'_LH_'+dweltypes['id']+'_'+allyear[y]]);                       } 
                        data['chart']=false;
                        datar.push(data); 
                    }
                }    
...
function showgrid(results) {
    var allyear = results['allyear'];
	var cv = new wijmo.collections.CollectionView(getdatadwellingfactorhhrural(results));
