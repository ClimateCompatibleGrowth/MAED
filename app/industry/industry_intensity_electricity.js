    ...
    jQuery.each(maintype, function(i, maintypes) {
        ...
        if (maintypes['ind'] == 'Y') {
            ...
            for (j = 0; j < typechunk.length; j++) {
                ...
                if (item != undefined && bjdata['AP_' + typechunk[j]] == 'Y') {
                    var data = new Array();
                    data['conf'] = 1;
            ...
            for (j = 0; j < typechunk.length; j++) {
                ...
                if (item != undefined && bjdata['AP_' + typechunk[j]] == 'Y') {
                    ...
                    for (var y = 0; y < allyear.length; y++) { data[' ' + allyear[y]] = isNumber(andata[typechunk[j] + '_' + allyear[y]]);}
                    data['chart']=false;
                    datar.push(data);
                }
            }
...
function showgrid(results) {
    var allyear = results['allyear'];
    var cv = new wijmo.collections.CollectionView(getdataindustryintensityelec(results));
